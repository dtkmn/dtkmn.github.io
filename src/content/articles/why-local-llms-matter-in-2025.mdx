...
That is why this essay belongs next to the [LLM AI Assistant](https://github.com/dtkmn/llm-ai-assistant), which is also listed with the supporting [AI systems projects](/projects/). The project is deliberately practical: document-grounded QA, local model execution, vector search, and a runnable interface. It is a private-AI boundary test, not just another chat UI.

The useful question is simple: can a user ask questions over private documents while keeping retrieval, model execution, and evidence close enough to inspect? Once that local system connects to external tools, the next boundary becomes [MCP](/posts/mcp-the-usb-c-of-ai-integrations/) and the operational controls in [MCP ZAP Server](/projects/mcp-zap-server/). Once the system can affect customer state, the next test is [customer outcomes](/posts/simulate-customer-outcomes-before-shipping-ai-agents/).
